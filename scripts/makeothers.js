...
fs.readdir(path.join(__dirname, "../emojis"), (err, files) => {
    if (err) throw err;

    for (const folder of files) {
        if (folder.includes(".")) continue;

        const folderPath = path.join(__dirname, "../emojis/", folder);
        glob.glob(path.join(folderPath, "/png") + `/**/*.png`).then(async (relPaths) => {
            // avif folder
            const folders = [
                { path: "avif" },
            ...
            for (const relPngPath of relPaths) {
                ...
                for (const folder of folders) {
                    const newPath = path.join(path.join(folderPath, folder.path), newFileName);
